...
public class LectorWebFacade {
    ...
    public void createLector(final LectorRequestDto request) {
        lectorValidator.validateForCreate(request);

        final Lector lector = lectorMapper.toEntity(request);
        lectorService.save(lector);

        log.info("Created lector with ID: {}", lector.getId());
    }
    ...
    public void updateLector(final UUID id,
        ...
        lectorValidator.validateForUpdate(request);

        final Lector lector = getLectorOrThrow(id);
        lectorMapper.updateEntityFromDto(request, lector);
        lectorService.save(lector);

        log.info("Updated lector with ID: {}", id);
    }

    @Transactional
    public void deleteLector(final UUID id) {
        final Lector lector = getLectorOrThrow(id);

        lectorService.delete(lector);

        log.info("Deleted lector with ID: {}", id);
    }
